...
GPU использует двухэтапный подход:
1. **GPU фильтрация** — проверка верхних 36 бит с permissive фильтром (`create_check(12)`)
2. **CPU финализация** — для каждого кандидата проверяются все 4096 комбинаций нижних бит
...
│  │                                                           │  │
│  │  Checks создаются с create_check(12) - permissive режим  │  │
│  │  (учитывает что нижние 12 бит могут быть любыми)        │  │
│  │                                                           │  │
...
│                              │                                  │
│                              ▼                                  │
│                    ~100-1000 кандидатов (upper 36 bits)        │
└─────────────────────────────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────┐
...
│      // Проверка всех 4096 комбинаций нижних бит              │
│      for lower in 0..4096:                                     │
│          full_seed = candidate | lower                         │
│                                                                 │
│          // Точная проверка с create_check(0)                  │
│          if all primary_checks pass:                           │
│              CrossComparison::run(full_seed)                   │
│              → World Seed                                      │
└─────────────────────────────────────────────────────────────────┘
```
